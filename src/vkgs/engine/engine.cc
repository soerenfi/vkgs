...
class Engine::Impl {
   ...
   public:
    Impl() {
        ...
        splat_visible_point_count_ =
            vk::Buffer(context_, sizeof(uint32_t),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
                           VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
        splat_draw_indirect_ =
        ...
        // preallocate splat storage
        {
            ...
            splat_storage_.key = vk::Buffer(
                context_, MAX_SPLAT_COUNT * sizeof(uint32_t),
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
            splat_storage_.index = vk::Buffer(
                context_, MAX_SPLAT_COUNT * sizeof(uint32_t),
                VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);
    ...
    void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width,
                           int height) {
        wd->Surface = surface;
        ...
        vkGetPhysicalDeviceSurfaceSupportKHR(
            context_.physical_device(), context_.graphics_queue_family_index(), wd->Surface, &res);
        if (res != VK_TRUE) {
...
#endif
        wd->PresentMode =
            ImGui_ImplVulkanH_SelectPresentMode(context_.physical_device(), wd->Surface,
    ...
    void Run() {
        ...
        VkResult err = glfwCreateWindowSurface(context_.instance(), window_, NULL, &surface);
        check_vk_result(err);
        ...
        while (!glfwWindowShouldClose(window_) && !terminate_) {
            ...
            //       glm::radians(30.0f);  // Steering rate in radians per
            //       second
            ...
            //   // Vehicle parameters
            //   const float wheelbase = 2.5f;  // Distance between front and
            //   rear axles const float max_steering_angle =
            ...
            //       angular_velocity * delta_time, glm::vec3(0.0f,
            //       0.0f, 1.0f));
            ...
            // Present();
            // Calculate frame duration and sleep if necessary to limit to 30
            // FPS auto frame_end_time =
   ...
   private:
    void PreparePrimitives() {
        ...
        for (int i = -grid_size; i <= grid_size; ++i) {
            ...
            // old:  (x=-1, y=0, z=t), (x=1, y=0, z=t), (x=t, y=0, z=-1), (x=t,
            // y=0, z=1) new:  (x=-1, y=t, z=0), (x=1, y=t, z=0), (x=t, y=-1,
            // z=0), (x=t, y=1, z=0)
        ...
        splat_index_buffer_ =
            vk::Buffer(context_, splat_index.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT);

        axis_.position_buffer =
            vk::Buffer(context_, axis_position.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        axis_.color_buffer =
            vk::Buffer(context_, axis_color.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        axis_.index_buffer =
            vk::Buffer(context_, axis_index.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT);

        grid_.position_buffer =
            vk::Buffer(context_, grid_position.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        grid_.color_buffer =
            vk::Buffer(context_, grid_color.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        grid_.index_buffer =
            vk::Buffer(context_, grid_index.size() * sizeof(float),
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT);
    ...
    void RenderCamera() {
        ...
        //         ToTranslationMatrix4(translation_ + gt) * glm::toMat4(rotation_ * lq) *
        //         ToTranslationMatrix4(lt);
        ...
        // update descriptor
        descriptors_[frame_index].gaussian.Update(0, splat_info_buffer_,
                                                  splat_info_buffer_.offset(frame_index),
                                                  splat_info_buffer_.element_size());

        descriptors_[frame_index].splat_instance.Update(0, splat_draw_indirect_, 0,
        ...
        if (loaded_point_count_ != 0) {
            ...
            descriptors_[frame_index].gaussian.Update(4, splat_storage_.sh, 0,
                                                      loaded_point_count_ * 48 * sizeof(float));

            descriptors_[frame_index].splat_instance.Update(
                1, splat_storage_.instance, 0, loaded_point_count_ * 10 * sizeof(float));
        ...
        if (!progress.buffer_barriers.empty()) {
            ...
            vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_COMPUTE, compute_pipeline_layout_, 1,
                                    1, &descriptor, 0, NULL);

            descriptor = descriptors_[frame_index].ply;
            vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_COMPUTE, compute_pipeline_layout_, 3,
                                    1, &descriptor, 0, NULL);
        ...
        if (loaded_point_count_ != 0) {
            ...
            // make visiblePointCount available for next transfer commands
            {
                ...
                barrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
                vkCmdPipelineBarrier(cb, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                     VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 1, &barrier, 0, NULL, 0,
                                     NULL);
            }
            ...
            // visible point count to CPU
            {
                ...
                vkCmdCopyBuffer(cb, splat_visible_point_count_, visible_point_count_cpu_buffer_, 1,
                                &region);
            }
            ...
            // radix sort
            {
                ...
                vrdxCmdSortKeyValueIndirect(cb, sorter_, loaded_point_count_,
                                            ...
                                            splat_storage_.index, 0, sort_storage_, 0, NULL, 0);
            ...
            // draw
            {
                ...
                vkCmdPipelineBarrier(
                    ...
                    VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT | VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0, 1,
    ...
    void UiFrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data) {
        ...
        {
            err = vkWaitForFences(context_.device(), 1, &fd->Fence, VK_TRUE,
    ...
    void RenderUI() {
        ...
        if (fb_width > 0 && fb_height > 0 &&
            ...
            ImGui_ImplVulkanH_CreateOrResizeWindow(
                context_.instance(), context_.physical_device(), context_.device(), &main_window_,
                context_.graphics_queue_family_index(), NULL, width_, height_, 3);
        ...
        {
            ImGui::Begin("Gaussian Splat Renderer");
        ...
        }
        {
            ...
            ImGui::Image((ImTextureID)(intptr_t)color_attachment_.getDescriptorSet(),
    ...
    void DrawNormalPass(VkCommandBuffer cb, uint32_t frame_index, uint32_t width, uint32_t height,
        ...
        // draw axis and grid
        {
            ...
            model[1][1] = 10.f;
            model[2][2] = 10.f;
            vkCmdPushConstants(cb, graphics_pipeline_layout_, VK_SHADER_STAGE_VERTEX_BIT, 0,
                               sizeof(model), &model);
