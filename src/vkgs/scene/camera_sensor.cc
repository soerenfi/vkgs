...
void CameraSensor::SetRenderSize(uint32_t width, uint32_t height) {
  width_ = width;
  height_ = height;
}

void CameraSensor::SetPosition(const glm::mat4& transform) {
  // Extract translation
...
void CameraSensor::SetPosition(const glm::vec3& position) {
  center_ = position;
  std::cout << "Center: (" << center_.x << ", " << center_.y << ", "
            << center_.z << ")" << std::endl;
}

void CameraSensor::SetOrientation(const glm::quat& orientation) {
...
glm::quat CameraSensor::Orientation() const {
...
void CameraSensor::SetFov(float fov) {
  // dolly zoom
  r_ *= std::tan(fovy_ / 2.f) / std::tan(fov / 2.f);
  fovy_ = fov;
}

glm::mat4 CameraSensor::ProjectionMatrix() const {
...
// --------------------------------------------------------------------
glm::mat4 CameraSensor::ViewMatrix() const {
...
//       z = r cos(phi)
// --------------------------------------------------------------------
glm::vec3 CameraSensor::Eye() const {
...
// --------------------------------------------------------------------
void CameraSensor::Rotate(float x, float y) {
...
// --------------------------------------------------------------------
void CameraSensor::Translate(float x, float y, float z) {
  // Local "forward"
  ...
  // Local CameraSensor up = right x forward
  glm::vec3 u = glm::cross(r, f);

  // Move center_ in local CameraSensor space
